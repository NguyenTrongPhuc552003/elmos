...
if [ -f /mnt/modules/module.cfg ]; then
    echo "  [GUEST] Processing module queues..."
    # Source the config file generated by the host
    . /mnt/modules/module.cfg
    
    # Process INSMOD queue
    for mod in \$MODULE_INS; do
        if [ "\$mod" = "*" ]; then
            for m in /mnt/modules/*/*.ko; do insmod "\$m"; done
        else
            insmod "/mnt/modules/\$mod/\$mod.ko" && echo "  [GUEST] Loaded \$mod"
        fi
    done
fi
